...
```
By default folder name is used as project name, which is recommended because `.vscode/launch.json` uses `${workspaceFolderBasename}`. This way we can omit `ddev config` and it also uses foldername as default name.

### 2. Set up DDEV environment
```bash
ddev start
```
...
On dev/build database structure will be generated. Adjust default locale in `app/_config.php` before:
```php
