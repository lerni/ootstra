...
<div class="typography">
	<% if $Teasers %>
		<% if $ShowAsSlider %>
			...
			<div class="swiper-container multiple card" id="general-swiper-{$ID}">
		...
		<% if $Layout == "third" %>
			<% loop $Teasers.Sort(TeaserSortOrder) %>
				...
				<% if $Link %><a href="$Link.URL" <% if $Link.OpenInNew %>target="_blank" rel="noopener noreferrer" <% end_if %><% else %><div <% end_if %>class="card<% if $Up.ShowAsSlider %> swiper-slide<% end_if %>">
					...
					<% if $Link %><span class="link forth">{$Link.Title}</span><% end_if %>
