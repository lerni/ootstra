	...
	<% if $Teasers %>
			...
			<div class="teasers halve">
				<% loop $Teasers.Sort(TeaserSortOrder) %>
					<% if $RelatedPageID %><a href="$RelatedPage.Link" <% else %><span <% end_if %>class="teaser $Layout">
						<% if $Image %><figure><img width="$Image.FocusFillMax(480,400).Width()" height="$Image.FocusFillMax(480,400).Height()" src="$Image.FocusFillMax(480,400).URL" srcset="$Image.FocusFillMax(480,400).URL 1x, $Image.FocusFillMax(960,800).URL 2x" alt="$Title" /></figure><% end_if %>
						<div class="txt">
							<% if $Title %><h4>$Title</h4><% end_if %>
							<div class="accordion">
								<% if $Text %><p>$Text</p><% end_if %>
		...
		<% if $Layout == "full" %>
			<div class="teasers full">
				<% loop $Teasers.Sort(TeaserSortOrder) %>
					<% if $RelatedPageID %><a href="$RelatedPage.Link" <% else %><span <% end_if %>class="teaser $Layout">
						<% if $Image %><figure><img width="$Image.FocusFillMax(1400,600).Width()" height="$Image.FocusFillMax(1400,600).Height()" src="$Image.FocusFillMax(1400,600).URL" srcset="$Image.FocusFillMax(1400,600).URL 1x, $Image.FocusFillMax(2800,1200).URL 2x" alt="$Title" /></figure><% end_if %>
						<div class="txt">
							<% if $Title %><h4>$Title</h4><% end_if %>
							<div class="accordion">
								<% if $Text %><p>$Text</p><% end_if %>
