				...
				<% loop $Items %>
					<a href="$ScaleMaxWidth(1224).URL"
						...
						data-height="$ScaleMaxWidth(1224).Height">
							<img<% if not $IsFirst %> loading="lazy" <% end_if %> width="$ScaleMaxHeight(600).Width()" height="$ScaleMaxHeight(600).Height()" src="$ScaleMaxHeight(600).URL" alt="$Title" />
					</a>
				<% end_loop %>
			</div>
		</div>
			...
			<% loop $Items %>
				<li>
					<a href="$ScaleMaxWidth(1224).URL"
						...
						data-width="$ScaleMaxWidth(1224).Width"
						data-height="$ScaleMaxWidth(1224).Height">
						<img<% if not $IsFirst %> loading="lazy"<% end_if %> width="$ScaleMaxHeight(130).Width()" height="$ScaleMaxHeight(130).Height()" src="$ScaleMaxHeight(130).URL" srcset="$ScaleMaxHeight(130).URL 1x, $ScaleMaxHeight(260).URL 2x" alt="$Title" />
					</a>
				</li>
			<% end_loop %>
		</ul>
			...
			<% loop $Items %>
				<li>
					<a href="$ScaleMaxWidth(1224).URL"
						...
						data-width="$ScaleMaxWidth(1224).Width"
						data-height="$ScaleMaxWidth(1224).Height">
						<img<% if not $IsFirst %> loading="lazy"<% end_if %> width="$FocusFillMax(250,187).Width()" height="$FocusFillMax(250,187).Height()" src="$FocusFillMax(250,187).URL" srcset="$FocusFillMax(250,187).URL 1x, $FocusFillMax(500,374).URL 2x" alt="$Title" />
					</a>
				</li>
			<% end_loop %>
		</ul>
