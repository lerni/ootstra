    ...
    <div class="jobs teasers">
        <% loop $Items %>
            <a href='{$AbsoluteLink}' id="#{$Slug}" class="job teaser">
                <% if $HeaderImage %><figure><img src="$HeaderImage.FocusFillMax(340,160).URL" srcset="$HeaderImage.FocusFillMax(340,160).URL 1x, $HeaderImage.FocusFillMax(680,320).URL 2x" alt="$Title" /></figure><% end_if %>
                <div class="txt">
