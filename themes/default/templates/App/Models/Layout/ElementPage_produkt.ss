							...
							<div class="product-swiper swiper-container article" data-id="{$pos}" id="article-swiper{$pos}">
								<div class="swiper-wrapper">
									<% loop $Images.Sort("ImageSortOrder ASC") %>
										...
										<img <% if not $IsFirst %>loading="lazy" <% end_if %>height="480" width="480" src="$FocusFillMax(480,480).Convert('webp').URL" srcset="$FocusFillMax(480,480).Convert('webp').URL 1x, $FocusFillMax(960,960).Convert('webp').URL 2x" alt="$Title" />
										</div>
									<% end_loop %>
								</div>
