<% if not $Page.PreventHero && $Siteconfig.DefaultHeaderSlides.Count %><article class="element elementhero default full-width spacing-bottom-2<% if $Top.ClassName == 'App\Models\ElementPage' && $Top.ParentID != 0 %> breadcrumbs<% end_if %><% if $Page.SiteConfig.GlobalAlert %> global-alert<% end_if %>">
