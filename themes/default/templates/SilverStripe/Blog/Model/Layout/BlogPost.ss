	...
	<div class="prev-all-next inner">
		<% if $PrevNext('prev') %>
			<a href="{$PrevNext('prev').Link()}" class="prev">
				<img src="$PrevNext('prev').getDefaultOGImage(1).FocusFillMax(80,80).WebP.URL" height="80" width="80" alt="$PrevNext('prev').Title" loading="lazy">
		...
		<% if $PrevNext('next') %>
			<a href="{$PrevNext('next').Link()}" class="next">
				...
				<img src="$PrevNext('next').getDefaultOGImage(1).FocusFillMax(80,80).WebP.URL" height="80" width="80" alt="$PrevNext('next').Title" loading="lazy">
			</a>
		<% else %>
