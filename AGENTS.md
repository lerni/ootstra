...
## Prompt Specs (`.prompts/`)
- Non-trivial tasks get a versioned spec under `.prompts/tasks/NNN-slug.md` (see `.prompts/README.md` and `.prompts/templates/feature-spec.md`). Refine the spec in place as the task evolves instead of losing prompt history to chat scrollback.
- Reference the spec from the commit message, e.g. `Prompt-Spec: .prompts/tasks/042-add-custom-silverstripe-block.md`.
- `.prompts/` is excluded from deploys via `clear_paths` in `deploy.php` — it stays in git but never ships to test/live.

## Idea Backlog (`NEXT.md`)
- `NEXT.md` at the repo root holds half-baked ideas/prompts that haven't been spec'd out yet — agents may append to it when a tangential idea surfaces during other work.
- When work actually starts on an item, create `.prompts/tasks/NNN-slug.md` from it and delete the line from `NEXT.md` — one source of truth, no dual-maintenance.
- `app/todo` is separate: the user's own notes, agents never write to it.
- Like `.prompts/`, `NEXT.md` is excluded from deploys via `clear_paths` in `deploy.php`.
...
- Email addresses in FE are automatically obfuscated per axllent/silverstripe-email-obfuscator
